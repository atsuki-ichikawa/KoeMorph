...
class TestKoeMorphDataset:
    ...
    def sample_data_dir(self, tmp_path):
        ...
        for i in range(3):
            ...
            audio = 0.1 * np.random.randn(8000).astype(
                np.float32
    ...
    def test_dataset_length_limiting(self, sample_data_dir):
        ...
        dataset = KoeMorphDataset(
            sample_data_dir, max_audio_length=0.25
        )  # 0.25 seconds
...
class TestCollateFn:
    ...
    def test_collate_masks(self):
        ...
        batch = [
            {"wav": torch.ones(100), "arkit": torch.ones(5, 52)},
            {"wav": torch.ones(200), "arkit": torch.ones(10, 52)},
        ]
...
class TestKoeMorphDataModule:
    ...
    def test_datamodule_train_only(self, data_dirs):
        ...
        dm = KoeMorphDataModule(
            train_data_dir=data_dirs["train"], batch_size=1, num_workers=0
        )
