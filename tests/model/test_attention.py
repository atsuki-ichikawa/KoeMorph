...
class TestMultiHeadCrossAttention:
    ...
    def test_attention_with_padding_mask(self):
        ...
        attention = MultiHeadCrossAttention(d_query=64, d_key=64, d_value=64, d_model=128, num_heads=2)
        ...
        assert torch.allclose(attn_weights[0, :, :, 15:], torch.zeros_like(attn_weights[0, :, :, 15:]))
        assert torch.allclose(attn_weights[1, :, :, 10:], torch.zeros_like(attn_weights[1, :, :, 10:]))
    ...
    def test_gradient_flow(self):
        ...
        attention = MultiHeadCrossAttention(d_query=32, d_key=32, d_value=32, d_model=64, num_heads=2)
    ...
    def test_invalid_head_dimension(self):
        ...
        with pytest.raises(ValueError, match="d_model.*must be divisible by num_heads"):
            MultiHeadCrossAttention(d_query=32, d_key=32, d_value=32, d_model=65, num_heads=8)
...
class TestMultiStreamAudioEncoder:
    ...
    def test_encoder_forward_concat(self):
        ...
        encoder = MultiStreamAudioEncoder(
            mel_dim=80, prosody_dim=4, emotion_dim=256, d_model=128, fusion_method="concat"
        )
    ...
    def test_encoder_forward_gate(self):
        ...
        encoder = MultiStreamAudioEncoder(
            mel_dim=80, prosody_dim=4, emotion_dim=256, d_model=128, fusion_method="gate"
        )
    ...
    def test_encoder_without_positional_encoding(self):
        ...
        encoder = MultiStreamAudioEncoder(
            mel_dim=80, prosody_dim=4, emotion_dim=256, d_model=128,
