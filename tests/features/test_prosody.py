...
class TestProsodyExtractor:
    ...
    def test_sine_wave_f0_detection(self):
        ...
        if voiced_frames.any():
            ...
            assert abs(f0_mean - freq) < 20, f"F0 detection error: {f0_mean:.1f} vs {freq:.1f} Hz"
    ...
    def test_noisy_audio_robustness(self):
        ...
        assert voicing.max() <= 1.0 and voicing.min() >= 0.0, "Voicing prob out of range"
...
class TestProsodyNormalizer:
    ...
    def sample_features(self):
        ...
        features[:, :, 1] = torch.linspace(-8, -3, seq_len).unsqueeze(0).repeat(batch_size, 1)
...
class TestProsodyValidation:
    ...
    def test_valid_features(self):
        ...
        features[:, :, 0] = torch.linspace(100, 200, seq_len).unsqueeze(0).repeat(batch_size, 1)  # F0
        features[:, :, 1] = torch.linspace(-8, -3, seq_len).unsqueeze(0).repeat(batch_size, 1)   # Energy
    ...
    def test_statistics_computation(self):
        ...
        # Set known values
        f0_values = torch.tensor([100., 150., 200.])
        features[0, :3, 0] = f0_values  # First 3 frames voiced
        features[:, :, 3] = torch.where(
            torch.arange(seq_len).unsqueeze(0) < 3, 
