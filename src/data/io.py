...
class ARKitDataLoader:
    ...
    def load_sample(
        self, jsonl_path: Union[str, Path], wav_path: Union[str, Path]
        ...
        return {"wav": torch.from_numpy(audio), "arkit": torch.from_numpy(blendshapes)}
        ...
        with open(jsonl_path, "r") as f:
            for line_no, line in enumerate(f, 1):
                ...
                if not np.all((bs_array >= 0) & (bs_array <= 1)):
                    warnings.warn(
                        f"Blendshape values outside [0,1] range at line {line_no}"
                    )
    ...
    def load_batch(
        self, file_pairs: List[Tuple[Union[str, Path], Union[str, Path]]]
...
def validate_data_consistency(
    samples: List[Dict[str, torch.Tensor]], tolerance: float = 0.05
) -> Dict[str, bool]:
    """
    ...
    for i, sample in enumerate(samples):
        ...
        relative_error = abs(audio_duration - bs_duration) / max(
            audio_duration, bs_duration
        )
