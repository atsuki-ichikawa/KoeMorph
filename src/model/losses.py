...
class GaussianFaceLoss(nn.Module):
    ...
    def forward(
        ...
        if self.temporal_weight > 0 and prev_pred is not None and prev_target is not None:
        ...
        if self.velocity_weight > 0 and prev_pred is not None and prev_target is not None:
        ...
        # Additional metrics
        metrics.update(self._compute_additional_metrics(pred_blendshapes, target_blendshapes))
    ...
    def _compute_smoothness_loss(self, blendshapes: torch.Tensor) -> torch.Tensor:
        ...
        diff = torch.diff(blendshapes, dim=1)  # Differences between adjacent blendshapes
...
class PerceptualBlendshapeLoss(nn.Module):
    ...
    def _compute_audiovisual_loss(
        ...
        mouth_activation = blendshapes[:, self.mouth_group].mean(dim=1)  # Average mouth activation
        
        if audio_features.dim() == 3:  # (B, T, D)
            audio_energy = audio_features.norm(dim=2).mean(dim=1)  # Average energy per batch
        else:
        ...
        correlation = F.cosine_similarity(mouth_norm.unsqueeze(0), audio_norm.unsqueeze(0))
...
class LandmarkConsistencyLoss(nn.Module):
    ...
    def forward(
        ...
        target_landmarks = torch.matmul(target_blendshapes, self.bs_to_landmark_weights.T)
...
def compute_lip_sync_metrics(
    ...
    if pred_mouth_activity.std() > 1e-6 and target_mouth_activity.std() > 1e-6:
        ...
        metrics['mouth_correlation'] = mouth_corr.item() if not torch.isnan(mouth_corr) else 0.0
    else:
    ...
    if audio_features is not None:
        ...
        if audio_energy.std() > 1e-6:
            av_corr = torch.corrcoef(
                torch.stack([pred_mouth_activity, audio_energy])
            )[0, 1]
            metrics['audiovisual_sync'] = av_corr.item() if not torch.isnan(av_corr) else 0.0
        else:
