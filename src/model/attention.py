...
class MultiHeadCrossAttention(nn.Module):
    ...
    def forward(
        ...
        if batch_size != batch_size_k:
            raise ValueError(f"Batch size mismatch: query {batch_size}, key {batch_size_k}")
        ...
        Q = Q.view(batch_size, q_len, self.num_heads, self.head_dim).transpose(1, 2)  # (B, H, Q, d_head)
    ...
    def _compute_attention(
        ...
        if self.window_size is not None:
            window_mask = self._create_window_mask(Q.size(-2), seq_len, self.window_size, Q.device)
    ...
    def _create_causal_mask(self, q_len: int, k_len: int, device: torch.device) -> torch.Tensor:
    ...
    def _create_window_mask(self, q_len: int, k_len: int, window_size: int, device: torch.device) -> torch.Tensor:
...
class BlendshapeQueryEmbedding(nn.Module):
    ...
    def forward(
        ...
        queries = self.query_embeddings.unsqueeze(0).repeat(batch_size, 1, 1)  # (B, 52, d_query)
        ...
        if self.use_conditioning and prev_blendshapes is not None:
            ...
            conditioning = conditioning.unsqueeze(1).repeat(1, self.num_blendshapes, 1)  # (B, 52, d_query)
