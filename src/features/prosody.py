...
class ProsodyExtractor(nn.Module):
    ...
    def _extract_prosody_single(self, audio: np.ndarray) -> torch.Tensor:
        ...
        # Stack features
        features = np.stack([f0_resampled, energy_resampled, vad_resampled, voiced_resampled], axis=1)
    ...
    def _extract_vad(self, audio: np.ndarray, log_energy: np.ndarray) -> np.ndarray:
        ...
        energy_norm = (log_energy - log_energy.min()) / (log_energy.max() - log_energy.min() + 1e-8)
    ...
    def _resample_to_target_fps(self, features: np.ndarray, audio_length: int) -> np.ndarray:
        ...
        if current_frames > 1:
            ...
            features_resampled = np.full(target_frames, features[0] if current_frames > 0 else 0.0)
...
class ProsodyNormalizer(nn.Module):
    ...
    def denormalize(self, features_norm: torch.Tensor) -> torch.Tensor:
        ...
        if voiced_mask.any():
            if self.f0_log_transform:
                f0_log_denorm = f0_norm * (torch.log(self.f0_std + self.eps) + self.eps) + torch.log(self.f0_mean + self.eps)
                f0_denorm = torch.exp(f0_log_denorm) - self.eps
                features[..., 0] = torch.where(voiced_mask, f0_denorm, torch.zeros_like(f0_norm))
            else:
...
def validate_prosody_features(features: torch.Tensor) -> dict:
    ...
    if voiced_frames.any():
        ...
        if results['stats']['f0_min'] < 50 or results['stats']['f0_max'] > 500:
            results['warnings'].append(f"F0 range unusual: {results['stats']['f0_min']:.1f}-{results['stats']['f0_max']:.1f} Hz")
    else:
